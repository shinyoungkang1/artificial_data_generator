---
name: legal-contract-docs-synthetic-data
description: >-
  Generate synthetic legal contract document artifacts with configurable OCR
  scan degradation. Produces PDF, clean PNG, and noisy PNG outputs with a
  manifest.json index. Use when you need realistic scanned contract documents
  to stress-test OCR extraction pipelines, clause parsing accuracy, or legal
  document intelligence systems. Do NOT use when you need structured tabular
  contract data -- use legal-contract-metadata-synthetic-data instead. Do NOT
  use when you need amendment tracking records -- use
  legal-amendment-chain-synthetic-data instead.
---
...
This skill generates synthetic legal contract document artifacts designed for
OCR pipeline testing. Each generated document contains realistic contract
fields -- party names, contract types, governing law, financial terms, key
clauses, and signature blocks -- rendered as PDF and PNG images.
...
| Property | Value |
|----------|-------|
| Generator script | `scripts/generate_contract_docs.py` |
...
```bash
python skills/legal-contract-docs-synthetic-data/scripts/generate_contract_docs.py \
  --docs 75 \
  --seed 251 \
  --messiness 0.5 \
  --outdir ./skills/legal-contract-docs-synthetic-data/outputs
```
...
|------|------|---------|-------------|
| `--docs` | int | 75 | Number of documents to generate |
| `--seed` | int | 251 | Random seed for reproducibility |
| `--messiness` | float | 0.5 | Degradation intensity, clamped to [0.0, 1.0] |
| `--outdir` | str | `./skills/legal-contract-docs-synthetic-data/outputs` | Output directory path |
...
outputs/
├── pdf/
│   └── contract_{00001..N}.pdf
├── png_clean/
│   └── contract_{00001..N}.png
├── png_noisy/
│   └── contract_{00001..N}_noisy.png
└── manifest.json
```
...
|-------|--------|---------|
| Document ID | `LDOC-{06d}` | `LDOC-000001` |
| Contract ID | `LCON-{7 digits}` | `LCON-1400523` |
| Party A | Company name | `Acme Corp` |
| Party B | Company name | `TechVentures LLC` |
| Contract Type | Type label | `NDA` |
| Effective Date | `YYYY-MM-DD` | `2025-06-15` |
| Expiry Date | `YYYY-MM-DD` | `2027-03-22` |
| Governing Law | `State of {state}` | `State of Delaware` |
| Total Value | `$X,XXX.XX` | `$450,000.00` |
| Payment Terms | Term label | `Net 30` |
| Key Clause | Clause description | `Confidentiality: 3 years post-termination` |
| Signature Block | Blank lines for signatures | `Authorized Signatory (Party A): ___` |

### Financial derivation

Total value is randomly generated in the range [10000, 2000000]. All amounts
are displayed in currency format with commas and two decimal places.
...
```json
{
  "docs": [
    {
      "doc_id": "LDOC-000001",
      "pdf": "skills/.../outputs/pdf/contract_00001.pdf",
      "png_clean": "skills/.../outputs/png_clean/contract_00001.png",
      "png_noisy": "skills/.../outputs/png_noisy/contract_00001_noisy.png"
    }
  ],
  "count": 75
}
...
```bash
python skills/legal-contract-docs-synthetic-data/scripts/validate_docs.py \
  --dir ./skills/legal-contract-docs-synthetic-data/outputs
```
...
| **legal-contract-docs-synthetic-data** (this) | Scanned contract artifacts | PDF, PNG with OCR noise |

**Why 3 skills?** Legal pipelines ingest contract metadata tables, track
amendment chains, and parse scanned contract documents. Contract docs are the
hardest extraction target -- OCR degradation on party names, dollar amounts,
and dates creates failures that structured data alone cannot simulate.

**Recommended combo:** Generate contract metadata + amendment chain for
structured data, then contract docs referencing those contract numbers to test
whether your OCR pipeline can reconcile scanned values against the ground-truth
tables.
...
- `references/domain-notes.md` -- Detailed field layout, degradation
  parameters, and real-world context for legal contract documents.
